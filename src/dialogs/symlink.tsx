...
const CreateLinkModal = ({ dialogResult, path, selected } : {
    ...
    const createLink = async () => {
        ...
        const is_dir = symlinkName.startsWith('/') || symlinkName.startsWith('..');
        let owner = null;

        if (superuser.allowed && user && cwdInfo) {
            let folder_info = cwdInfo;
            if (is_dir) {
                folder_info = await fsinfo(dirname(symlinkName), ["user", "group", "mode"], { superuser: "try" });
            }

            const candidates = [...get_owner_candidates(user, folder_info)];
            if (candidates !== null) {
                owner = candidates[0];
            }
        }
          ...
          footer={
              <>
                  <Button
                    variant="primary"
                    onClick={() => {
                        const filename_valid = checkLinkName(symlinkName, cwdInfo?.entries || {}, selected);
                        if (filename_valid) {
                            setSymlinkError(filename_valid);
                        } else {
                            createLink();
                        }
                    }}
            ...
            >
                ...
                <FormGroup fieldId="symlink-input" label={_("Symlink name")}>
                    <TextInput
                      ...
                      value={symlinkName}
                      onChange={(_, val) => {
                          const filename_valid = checkLinkName(val, cwdInfo?.entries || {}, selected);
                          setSymlinkError(filename_valid);
