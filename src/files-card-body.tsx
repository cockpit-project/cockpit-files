...
export const FilesCardBody = ({
	...
	return (
		<div id={files_parent_id} className="fileview-wrapper" ref={folderViewRef}>
			{contextMenu}
			...
			{sortedFiles.length !== 0 && (
				...
				>
					<Thead>
						<Tr>
							<Th sort={sortColumn(0)} className="col-name">
								{_("Name")}
							</Th>
							<Th sort={sortColumn(1)} className="col-size">
								{_("Size")}
							</Th>
							<Th sort={sortColumn(2)} className="col-date" modifier="nowrap">
								{_("Modified")}
							</Th>
						</Tr>
					</Thead>
...
const Row = React.memo(function Item({
	...
	return (
		<Tr className={className} data-item={file.name}>
			...
			<Td className="item-size pf-v5-m-tabular-nums" dataLabel="size">
