	...
	return (
		<CardHeader className="card-actionbar">
			<CardTitle component="h2" id="files-card-header">
				<TextContent>
					<Text component={TextVariants.h2}>{_("Directories & files")}</Text>
				</TextContent>
			</CardTitle>
			<Flex
			...
			>
				<SearchInput
					placeholder={_("Filter directory")}
					value={currentFilter}
					onChange={onFilterChange}
					onClear={(event) =>
						onFilterChange(event as React.FormEvent<HTMLInputElement>, "")
					}
				/>
				<ViewSelector
					isGrid={isGrid}
				...
				<MenuToggle
					...
					splitButtonOptions={{
						...
						items: [
							<MenuToggleAction
								aria-label={
									isGrid ? _("Display as a list") : _("Display as a grid")
								}
