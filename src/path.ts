...
export class Path {
    ...
    async read(options?: ReadOptions & { binary: true }): Promise<{ content: Uint8Array, tag: string }>
    async read(options?: ReadOptions & { binary: false }): Promise<{ content: string, tag: string }>
    async read(options?: ReadOptions): Promise<{ content: StrOrBytes, tag: string }> {
        ...
        const channel = new Channel<StrOrBytes>({ ...global_options, ...options, payload: 'fsread1' });
        ...
        return new Promise((resolve, reject) => {
            channel.on('close', message => {
                if (message.problem) {
                    ...
                    resolve({ tag: message.tag as string, content: join_data(data, channel.binary) });
                }
            });
        });
    }
